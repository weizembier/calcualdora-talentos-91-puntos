...
talent[i] = [0, "Blood Aura", 2, 3, 7]; i++;
talent[i] = [0, "Sudden Doom", 5, 2, 8]; i++;
...
talent[i] = [1, "Glacier Rot", 2, 2, 1]; i++;
...
talent[i] = [1, "Runic Power Mastery", 3, 2, 4]; i++;
talent[i] = [1, "Killing Machine", 5, 3, 4]; i++;
talent[i] = [1, "Frigid Dreadplate", 3, 2, 5]; i++;
talent[i] = [1, "Chill of the Grave", 2, 3, 5]; i++;
...
talent[i] = [1, "Endless Winter", 2, 4, 6]; i++;
talent[i] = [1, "Howling Blast", 1, 2, 7]; i++;
talent[i] = [1, "Frost Aura", 2, 3, 7]; i++;
talent[i] = [1, "Chilblains", 3, 4, 7, [getTalentID("Endless Winter"),2]]; i++;
talent[i] = [1, "Blood of the North", 5, 2, 8]; i++;
...
talent[i] = [1, "Hungering Cold", 1, 2, 11]; i++;
treeStartStop[t] = i -1;
t++;

//Unholy talents
talent[i] = [2, "Vicious Strikes", 2, 1, 1]; i++;
talent[i] = [2, "Morbidity", 3, 2, 1]; i++;
...
talent[i] = [2, "Shadow of Death", 1, 4, 4]; i++;
talent[i] = [2, "Summon Gargoyle", 1, 1, 5]; i++;
talent[i] = [2, "Impurity", 5, 2, 5]; i++;
talent[i] = [2, "Dirge", 2, 3, 5]; i++;
talent[i] = [2, "Magic Suppression", 5, 2, 6]; i++;
talent[i] = [2, "Reaping", 3, 3, 6]; i++;
talent[i] = [2, "Master of Ghouls", 1, 4, 6, [getTalentID("Shadow of Death"),1]]; i++;
talent[i] = [2, "Desecration", 5, 1, 7]; i++;
talent[i] = [2, "Anti-Magic Zone", 1, 2, 7, [getTalentID("Magic Suppression"),5]]; i++;
talent[i] = [2, "Unholy Aura", 2, 3, 7]; i++;
talent[i] = [2, "Night of the Dead", 2, 1, 8]; i++;
...
talent[i] = [2, "Unholy Blight", 1, 2, 11]; i++;
treeStartStop[t] = i -1;
t++;
...
rank[i] = [
"<span style=text-align:left;float:left;>1 Unholy<br></span><br><span style=text-align:left;float:left;>Instant<br></span><span style=text-align:right;float:right;>2 min cooldown</span><br> Places a large, stationary Anti-Magic Zone that reduces spell damage done to party or raid members inside it by 75%. The Anti-Magic Zone lasts for 30 sec or until it absorbs 111,376 spell damage."
		];
i++;
